...
class BilliardWorld:
    def __init__(self):
        ...
        # v2: mild per-episode physics randomization (still pool-like)
        self.table_profile = random.choices(
            ["standard", "slick", "grippy", "bouncy"],
            weights=[0.50, 0.15, 0.20, 0.15],
            k=1,
        )[0]
        profile_params = {
    ...
    def _sample_spawn_position(self, margin, w, h, r, placed):
        ...
        for _ in range(100):
            ...
            for px, py in placed:
                ...
                min_sep = r * (2.2 if self.spawn_mode in ("clustered", "two_cluster", "break_setup") else 2.5)
                if dist < min_sep:
                    valid = False
                    break
            if valid:
...
class PoolBot:
    ...
    def _randomize_personality(self):
        ...
        # v2: sample from gameplay archetypes instead of forcing pocket-heavy behavior.
        self.style = random.choices(
            ["scorer", "collider", "banker", "chaos", "sniper", "tapper"],
            weights=[0.28, 0.20, 0.12, 0.20, 0.10, 0.10],
            k=1,
        )[0]
        profiles = {
    ...
    def _pick_shoot_direction(self):
        ...
        if roll < t1:
            ...
            if other_balls:
                ...
                power = min(200, max(70, dist * 1.55)) * random.uniform(0.85, 1.25) * self.power_mult
