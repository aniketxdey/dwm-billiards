...
# golf/local_run/run_golf_pipeline.sh. The "Start" button in the browser sends
...
DIT_CFG="${DIT_CFG:-${REPO_ROOT}/golf/local_run/dit_golf_df.yaml}"
DIT_CKPT="${DIT_CKPT:-${REPO_ROOT}/golf/local_run/wm_runs/dit_golf_df_run01/checkpoints/ckpt_120000000.pt}"
VAE_CKPT="${VAE_CKPT:-${REPO_ROOT}/golf/local_run/vae_runs/vae_golf_run01/checkpoints/ckpt_4000000.pt}"
LATENTS_DIR="${LATENTS_DIR:-${REPO_ROOT}/golf/local_run/latents/golf_main/shards}"
...
for p in "${WM_INF_DEFAULT_CHECKPOINT}" "${WM_INF_DEFAULT_VAE_CHECKPOINT}"; do
  if [ ! -f "${p}" ]; then
    ...
    echo "         (run golf/local_run/run_golf_pipeline.sh first, or override the path)" >&2
  fi
done
